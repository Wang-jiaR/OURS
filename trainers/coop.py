...
#     model = model.float()  # 添加此行，确保模型权重为单精度
#
#     return model
#
#
...
#         ])
#         # return torch.stack([aug(img) for img in image])
#         # 逐个样本增强
#         augmented_images = []
#         for img in image:
#             img_pil = T.ToPILImage()(img.cpu())  # 转为PIL图像
#             img_aug = aug(img_pil)  # 应用增强
#             img_tensor = T.ToTensor()(img_aug).to(image.device).type(image.dtype)  # 显式指定数据类型
...
#         if self.cfg.TEST.MEASURE_COMPLEXITY:
#             self.measure_complexity()
#
#     def measure_complexity(self):
#         """测量模型的计算复杂度（FLOPs）、参数量和推理速度（FPS）"""
...
#         dummy_backdoor = torch.BoolTensor([False]).to(device)
#
#         # 测量参数量
...
#
#         # 测量FLOPs
#         macs, params = profile(model, inputs=(dummy_image, dummy_backdoor), verbose=False)
#         print(f"\n计算量统计 (FLOPs):")
#         print(f"MACs: {macs / 1e9:.2f} GMACs")
#         print(f"FLOPs: {macs * 2 / 1e9:.2f} GFLOPs")  # 1 MAC = 2 FLOPs
#
#         # 测量推理速度 (FPS)
#         num_repeats = 100
#         warmup = 10
...
#                 _ = model(dummy_image, dummy_backdoor)
#
#         # 正式测量
...
#
#         fps = num_repeats / elapsed
#         latency = elapsed / num_repeats * 1000  # 毫秒
#
#         print(f"\n推理速度统计 (batch_size=1):")
...
#             # print(f"backdoor_tag: {backdoor_tag}")
#             # print(f"filtered feat shape: {filtered_feat.shape}")
#             # print(f"filtered feat shape: {feat[backdoor_tag].shape}")
#             # print(f"filtered label shape: {label[~backdoor_tag].shape}")
#             #
#             lambda_clean = 1.0
#             lambda_adv = 1.0
#             lambda_oti = 1.0
...
#
#             xent = CrossEntropyLabelSmooth(num_classes=80)
#
#             loss_clean = None
#             loss_adv = None
#             TRI_LOSS = None
...
#             # if clean_exists:
#             #     clean_tag = ~backdoor_tag
#             #     anchor_clean = original_feat[clean_tag]
#             #     positive_clean = augmented_feat[clean_tag]
#             #     labels_clean = label[clean_tag]
#             #
#             #     # 随机选择负样本
#             #     neg_indices_clean = []
...
#             #         neg_indices_clean.append(neg_idx)
#             #     negative_clean = original_feat[torch.tensor(neg_indices_clean)]
#             #
#             #     # 计算Triplet Loss
#             #     triplet_loss_clean = TripletLoss()(anchor_clean, positive_clean, negative_clean)[0]
#             #
#             #     # 计算各类损失
#             #     ID_LOSS_clean = xent(score[clean_tag], labels_clean)
#             #     I2TLOSS_clean = xent(i2tscore[clean_tag], labels_clean)
#             #     l3loss_clean = ID_LOSS_clean + triplet_loss_clean + I2TLOSS_clean
#             #
#             #     # 合并基础损失和新增损失
#             #     loss_clean = F.cross_entropy(output[clean_tag], labels_clean) + l3loss_clean
#
#             if backdoor_exists:
...
#                 tokenized_pseudo = clip.tokenize(pseudo_texts).to(anchor.device)  # [B, 77]
#
#                 clip_model_single = self.model.module if isinstance(self.model, nn.DataParallel) else self.model
#                 with torch.no_grad():
#                     embed = clip_model_single.text_encoder.token_embedding(tokenized_pseudo)
#                     embed = embed + clip_model_single.text_encoder.positional_embedding.type(embed.dtype)
#                     embed = embed.permute(1, 0, 2)
#                     embed = clip_model_single.text_encoder.transformer(embed)
#                     embed = embed.permute(1, 0, 2)
#                     embed = clip_model_single.text_encoder.ln_final(embed).type(embed.dtype)
#                     eot_idx = tokenized_pseudo.argmax(dim=-1)
#                     pseudo_text_features = embed[torch.arange(embed.size(0)), eot_idx] @ \
#                                            clip_model_single.text_encoder.text_projection
#
...
#
#                 # 合并损失（加权）
#                 # loss_adv = loss_adv + l3loss_adv + lambda_oti * oti_loss
#             if clean_exists and backdoor_exists:
#                 loss = 1 * lambda_clean * loss_clean + 1 * lambda_adv * loss_adv + 1 * l3loss_adv+ lambda_oti * oti_loss
#             elif clean_exists and not backdoor_exists:
#                 loss = 1 * loss_clean
#             elif not clean_exists and backdoor_exists:
#                 loss = 1 * loss_adv + 1 * l3loss_adv+ lambda_oti * oti_loss
#             else:
#                 raise ValueError(
#                     "No clean or backdoor images found. Check the backdoor tag assignments in Dataset class.")
#
#             # debug_info = f"Loss: {loss.item()}"
#             # if loss_clean is not None:
#             #     debug_info += f", Clean Loss: {loss_clean.item()}"
#             # if loss_adv is not None:
#             #     debug_info += f", Adv Loss: {loss_adv.item()}"
#             # if TRI_LOSS is not None:
#             #     debug_info += f", Triplet Loss: {TRI_LOSS.item()}"
#             # print(debug_info)
#
...
#     #     # 保存PromptLearner的context vector
#     #     ctx_save_path = os.path.join(
#     #         os.getcwd(),
#     #         'models',
#     #         'ctx_vectors',
#     #         f'ctx_{self.cfg.MODEL_NAME}_{self.cfg.DATASET_NAME}_s{self.cfg.SEED}.pt'
#     #     )
#     #     # 确保目录存在
#     #     os.makedirs(os.path.dirname(ctx_save_path), exist_ok=True)
#     #     torch.save(self.model.prompt_learner.ctx.data, ctx_save_path)
#     #     print(f"Context vector saved to {ctx_save_path}")
#     #     # return results_clean, results_backdoor
#
#     # def after_train(self):
#     #     # 先執行父類別的 after_train() 來進行測試
#     #     # results_clean, results_backdoor = super().after_train()
#     #
#     #     # 保存 PromptLearner 的 context vector
#     #     ctx_save_path = os.path.join(
#     #         os.getcwd(),
#     #         'models',
#     #         'ctx_vectors',
...
class CoOp(TrainerX):
    ...
    #         os.getcwd(),
    #         'features',
    #         f'features_{self.cfg.MODEL_NAME}_{self.cfg.DATASET_NAME}_s{self.cfg.SEED}.npz'
    #     )
    #     os.makedirs(os.path.dirname(feature_save_path), exist_ok=True)
    #     self.extract_features(self.test_loader, feature_save_path)
    #     self.plot_tsne(feature_save_path)
    #
    #     # 3. 评估干净样本
    #     num_samples = self.test_loader.dataset.num_samples
    #     self.test_loader.dataset.backdoor_tags = torch.zeros(num_samples)  # 全部设为干净
    #     results_clean = self.test()
    #
    #     # 4. 评估后门样本
    #     self.test_loader.dataset.backdoor_tags = torch.ones(num_samples)  # 全部设为后门
    #     results_backdoor = self.test()
    #
    #     # 5. 返回符合 print_results() 格式的列表
    #     return [results_clean, results_backdoor]
