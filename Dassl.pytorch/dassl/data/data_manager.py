...
def build_data_loader(
    ...
    # 1. 先构造 dataset，拿到过滤后的 data_source
    dataset = dataset_wrapper(cfg, data_source, transform=tfm, is_train=is_train)
    filtered_data_source = dataset.data_source

    # 2. 再基于过滤后的 data_source 构造采样器，长度一致
    sampler = build_sampler(
        sampler_type,
        cfg=cfg,
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        sampler=sampler,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
        drop_last=is_train and len(filtered_data_source) >= batch_size,
        pin_memory=(torch.cuda.is_available() and cfg.USE_CUDA)
    )
    assert len(data_loader) > 0
    return data_loader
...
class DatasetWrapper(TorchDataset):
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        ...
        # 1. 过滤无效路径
        self.data_source = [item for item in data_source if os.path.exists(item.impath)]
        filtered = len(data_source) - len(self.data_source)
        print(f"[DatasetWrapper] Filtered out {filtered} missing images.")
        ...
        if is_train:
            self.backdoor_tags = torch.from_numpy(
                np.random.choice([0, 1],
                                 size=(self.num_samples,),
                                 p=[1 - self.backdoor_percentage / 100,
                                    self.backdoor_percentage / 100])
            )
        else:
            self.backdoor_tags = torch.zeros(self.num_samples)
        ...
        interp_mode = INTERPOLATION_MODES[cfg.INPUT.INTERPOLATION]
        to_tensor = [
            T.Resize(cfg.INPUT.SIZE, interpolation=interp_mode),
            T.ToTensor(),
        ]
        if "normalize" in cfg.INPUT.TRANSFORMS:
