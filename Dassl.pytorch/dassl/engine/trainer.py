...
class SimpleTrainer(TrainerBase):
    ...
    def train(self):
        ...
        results = super().train(self.start_epoch, self.max_epoch)
        return results
    ...
    def after_epoch(self):
        # print(f"\n\nCTX\n\n{self.model.prompt_learner.ctx.abs().sum()}")
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            input, label = self.parse_batch_test(batch)
            # breakpoint()
            output = self.model_inference(input, batch["backdoor_tag"]==1)
            self.evaluator.process(output, label)
    ...
    # def model_inference(self, input, backdoor_tags):
    #     return self.model(input, backdoor_tags)
    def model_inference(self, input, backdoor_tags=None):
        if self.model.training:
            return self.model(input, backdoor_tags, label=None)  # 训练时传入 label=None
        else:
            return self.model(input, backdoor_tags)  # 测试时直接返回 logits
